...
const ViewJourney = () => {
  ...
  const sendData = (e) => {
    ...
    if (e.target.value === "monthly") {
      axios
        .get(
          `${process.env.REACT_APP_SERVER_URL}user/get-maps/${y}-${m}-${d}`,
          {
            headers: {
        ...
        .then((res) => {
          if (
            res.data.data &&
            res.data.data.diff.length > 0 &&
            res.data.data.tried.length === 0 &&
            res.data.data.achieve.length === 0
          ) {
      ...
      axios
        .get(
          `${process.env.REACT_APP_SERVER_URL}user/get-events/${y}-${m}-${d}`,
          {
            headers: {
              Authorization: cookies.jwt_token,
            },
          ...
          if (res.data.data.length > 0) {
            for (let i = 0; i < res.data.data.length; i++) {
              if (!label.includes(res.data.data[i].date)) {
                console.log("ent->", label, res.data.data[i].date);
          ...
          if (res.data.data.length > 0) {
            for (let i = 0; i < res.data.data.length; i++) {
              console.log(
                ...
                label.includes(res.data.data[i].date)
              );
              if (!label.includes(res.data.data[i].date)) {
  ...
  useEffect(() => {
    ...
    let y,
      m = month_earlier.getMonth() + 1,
      d = month_earlier.getDate();
    ...
    axios
      .get(`${process.env.REACT_APP_SERVER_URL}user/get-maps/${y}-${m}-${d}`, {
        headers: {
          Authorization: cookies.jwt_token,
        },
      })
