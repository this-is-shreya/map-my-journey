...
const AddEvent = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_SERVER_URL}user/get-specific-event/${
          y + "-" + m + "-" + d
        }`,
        {
          headers: {
  ...
  const CountChars = (a, e) => {
    ...
    else diff_d_span.current.innerHTML = "(" + e.target.value.length + "/150)";
  };
  const checkLength = (e) => {
  ...
  const handleSubmit = () => {
    if (new_ref.length === 0 && new_d.current.value !== "") {
      ...
      axios
        .post(
          `${process.env.REACT_APP_SERVER_URL}user/add-event`,
    ...
    <div>
      ...
      <div className="mx-auto col-md-4 col-md-offset-4">
        <Form className="mt-5 mb-5">
          ...
          <Form.Group className="mb-3">
            <Form.Label>Did you do something new?</Form.Label>
            <br />
            <select onChange={(e) => selectVal("n", e)} multiple>
              ...
              <option value="tried something">
                Just tried something different 🪁
              </option>
            </select>
            <br />
            <Form.Label className="mt-3">Describe it</Form.Label>
            <br />
            ...
            <Form.Control
              ...
              onKeyDownCapture={checkLength}
              onKeyUpCapture={(e) => CountChars("n", e)}
            />
            <span className="badge rounded-pill text-bg-light" ref={new_d_span}>
              (0/150)
            </span>
          </Form.Group>
          <hr />
          <Form.Group className="mb-3">
            <Form.Label>Did you achieve something?</Form.Label>
            <br />
            <select onChange={(e) => selectVal("a", e)} multiple>
              {/* <option value="--">Pick a category</option> */}
              <option value="biggest achievement">
                My biggest achievement so far 🏆
              </option>
              <option value="got appreciated">
                Got appreciated for my work 🌟
              </option>
              <option value="worked on goal">
                Finally worked on my goal(s) 🏁
              </option>
            ...
            <Form.Control
              ...
              placeholder="Describe"
              onKeyDownCapture={checkLength}
              onKeyUpCapture={(e) => CountChars("achieve", e)}
            />
            <span
          ...
          <hr />
          <Form.Group className="mb-3">
            <Form.Label>Faced any difficulties?</Form.Label>
            <br />
            ...
            <Form.Control
              ...
              placeholder="Describe"
              onKeyDownCapture={checkLength}
              onKeyUpCapture={(e) => CountChars("d", e)}
            />
            <span
              className="badge rounded-pill text-bg-light"
              ref={diff_d_span}
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
          You have already added an event for today, do you want to edit it?
        </Modal.Body>
        <Modal.Footer>
